...
- 4 new power-ups: Rocket Burst, Gravity Well, Vampire, EMP
  ...
  - Gravity: time-based pull toward seeker (10 s duration, decays with distance)
...
- Gravity visual feedback: pulsing amber rings on Seeker, red pull arrow from Hider → Seeker
...
- Constants: added ROCKET_SPEED_MULT, GRAVITY_PULL_BASE/MAX, GRAVITY_DURATION_MS, EMP_FREEZE_MS/FRAMES, VAMPIRE_BONUS, ORBIT_SPEED/RADIUS
- HUD badge colors per power-up type
- Map generation now spawns new orb types
- Gravity Well: switched from turn-based to time-based (10 s duration per activation); shows "GRAVITY EXPIRED" on timeout
